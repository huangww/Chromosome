...
for x in tauData:
    if np.isclose(x[0],10):
        ax.scatter(x[0], x[-1]/tau0, marker='s',\
                s=90, color='r', alpha=0.5)
    if np.isclose(x[0],20):
        ax.scatter(x[0], x[-1]/tau0, marker='s',\
                s=90, color='c', alpha=0.5)
    if np.isclose(x[0],50):
        ax.scatter(x[0], x[-1]/tau0, marker='s',\
                s=90, color='m', alpha=0.5)
...
axinset = plt.axes([0.58, 0.24, 0.3, 0.3])
Tset = [10, 20, 50]
colors = ['r', 'c', 'm']
for T,c in zip(Tset,colors):
    fname = 'Asep_acf_N100_T%g_0.dat'%T
    t, acf = np.loadtxt(fname)
    axinset.plot(t, acf, '-', color=c)

axinset.set_yscale('log')
axinset.set_xlabel(r'$\Delta t$')
axinset.set_ylabel('ACF')
