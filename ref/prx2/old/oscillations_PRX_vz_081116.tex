...
%\end{equation}
%which allows very efficient computations of the $g(E)=\pi\left(L/2,L/2,E\right)$. In addition, the generating function 
\begin{equation}
...
\label{eq:binomial}
\end{equation}
% By comparing Eqs.(\ref{eq:par_func}) (\ref{eq:generating}) and (\ref{eq:generating}) we can find an explicit and exact formula for the partition function of the polymer loop problem:
% \begin{equation}
% \mathcal{Z}\left(T\right) = \frac{\prod_{j=L/2+1}^{L} \sum_{i=0}^{j}e^{-i\Delta E/k_{B}T}}{\prod_{j=1}^{L/2} \sum_{i=0}^{j}e^{-i\Delta E/k_{B}T}}.
